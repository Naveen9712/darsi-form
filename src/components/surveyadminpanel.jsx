    ...
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "#f1f5f9" }}>
          {count} <span style={{ color: "#64748b", fontWeight: 400 }}>({p}%)</span>
        </span>
      </div>
...
// ─── VS Card for majority/craze comparisons ─────────────────
function VsCard({ title, icon, data, fieldKey, colorA = "#f97316", colorB = "#3b82f6", total }) {
  const entries = data || [];
  const a = entries[0];
  const b = entries[1];
  const aCount = +(a?.count || 0);
  const bCount = +(b?.count || 0);
  const aLabel = a?.[fieldKey] || "—";
  const bLabel = b?.[fieldKey] || "—";
  const aPct = pct(aCount, total);
  const bPct = pct(bCount, total);
  const leader = aCount >= bCount ? aLabel : bLabel;
      ...
      {entries.length === 0 ? (
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 18, alignItems: "stretch" }}>
            <div style={{
              flex: 1, background: "#020617", borderRadius: 12, padding: "16px 14px",
              border: `1px solid ${colorA}44`, textAlign: "center",
            }}>
              <p style={{ margin: "0 0 6px", fontSize: 11, color: "#64748b", fontFamily: "Noto Sans Telugu, sans-serif", lineHeight: 1.4 }}>{aLabel}</p>
              <p style={{ margin: "0 0 2px", fontSize: 34, fontWeight: 800, color: colorA, lineHeight: 1 }}>{aPct}%</p>
              <p style={{ margin: 0, fontSize: 11, color: "#475569" }}>{aCount} votes</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 32, flexShrink: 0 }}>
              <span style={{ fontSize: 16, color: "#334155", fontWeight: 800 }}>VS</span>
            </div>
            <div style={{
              flex: 1, background: "#020617", borderRadius: 12, padding: "16px 14px",
              border: `1px solid ${colorB}44`, textAlign: "center",
            }}>
              <p style={{ margin: "0 0 6px", fontSize: 11, color: "#64748b", fontFamily: "Noto Sans Telugu, sans-serif", lineHeight: 1.4 }}>{bLabel}</p>
              <p style={{ margin: "0 0 2px", fontSize: 34, fontWeight: 800, color: colorB, lineHeight: 1 }}>{bPct}%</p>
              <p style={{ margin: 0, fontSize: 11, color: "#475569" }}>{bCount} votes</p>
            </div>
          </div>

          {/* Split bar */}
          <div>
            <p style={{ margin: "0 0 6px", fontSize: 11, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              Split · <span style={{ color: "#f1f5f9" }}>{leader}</span> leads
            </p>
            <div style={{ height: 10, borderRadius: 99, overflow: "hidden", display: "flex" }}>
              <div style={{ width: `${aPct}%`, background: colorA, transition: "width 0.8s" }} />
              <div style={{ width: `${bPct}%`, background: colorB, transition: "width 0.8s" }} />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5 }}>
              <span style={{ fontSize: 11, color: colorA }}>{aPct}%</span>
              <span style={{ fontSize: 11, color: colorB }}>{bPct}%</span>
            </div>
          </div>
        </>
      )}
    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 420, position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <p style={{ margin: 0, fontSize: 14, color: "#475569" }}>Darsi Political Survey · D7 News</p>
        </div>
        ...
        <p style={{ textAlign: "center", marginTop: 20, fontSize: 12, color: "#1e293b" }}>
          D7 News · Darsi Political Survey Admin
        </p>
      </div>
    </div>
  );
...
function Dashboard({ onLogout }) {
  ...
  // ── Response table columns definition ──────────────────────
  const TABLE_HEADERS = [
    "#", "Mandal", "Village", "Age", "Gender",
    "2024 Vote", "Next Vote", "Gottipati", "Bachepalli",
    "Sarpanch", "Majority", "Craze", "Date",   // ← NEW
  ];
  ...
  const rowCells = (row) => [
    ...
    row.voted_2024 || "—",
    row.vote_next,
    row.gottipati_rating,
    row.bachepalli_rating,
    row.sarpanch_support,
    row.majority_who || "—",   // ← NEW
    row.craze_who    || "—",   // ← NEW
  ];
        ...
        <div style={{ padding: "0 20px 24px", borderBottom: "1px solid #1e293b" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 11, color: "#475569" }}>D7 News</p>
            </div>
          </div>
        </div>
          ...
          <>
            ...
            {tab === "overview" && stats && (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  ...
                  {/* ── NEW: quick preview of majority & craze ── */}
                  <VsCard
                    title="Majority Prediction"
                    icon="🏆"
                    data={stats.by_majority_who}
                    fieldKey="majority_who"
                    colorA="#f97316"
                    colorB="#3b82f6"
                    total={stats.total_responses}
                  />
                  <VsCard
                    title="Craze / Popularity"
                    icon="⭐"
                    data={stats.by_craze_who}
                    fieldKey="craze_who"
                    colorA="#a855f7"
                    colorB="#10b981"
                    total={stats.total_responses}
            ...
            {tab === "leaders" && stats && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                {[
                  { name: "గొట్టిపాటి లక్ష్మి",           key: "gottipati_rating" },
                  { name: "బాచేపల్లి శివప్రసాద్ రెడ్డి", key: "bachepalli_rating" },
                ].map((leader) => {
                  ...
                  return (
                    <StatCard key={leader.key} title={leader.name} icon="👤">
                ...
                {/* ── NEW: Majority & Craze VS cards ── */}
                <VsCard
                  title="Who Gets More Majority?"
                  icon="🏆"
                  data={stats.by_majority_who}
                  fieldKey="majority_who"
                  colorA="#f97316"
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                      {[
                        ...
                        <div key={section.label}>
                          <p style={{ margin: "0 0 10px", fontSize: 12, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                            {section.label}
                          </p>
                          {(section.data || []).map((item, i) => (
                            <BarStat
                              key={item[section.fieldKey]}
                              label={item[section.fieldKey]}
                              count={+item.count}
                              total={stats.total_responses}
                    ...
                    <div style={{ overflowX: "auto" }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                        ...
                        <tbody>
                          {filtered.length === 0 ? (
                            <tr>
                              <td colSpan={TABLE_HEADERS.length} style={{ padding: 40, textAlign: "center", color: "#475569" }}>
                                No responses found
                              </td>
                            </tr>
                          ) : filtered.map((row, i) => (
                            <tr key={row.id} style={{ borderTop: "1px solid #1e293b", background: i % 2 === 0 ? "transparent" : "#0a1020" }}>
                              <td style={{ padding: "10px 14px", color: "#475569" }}>{row.id}</td>
                              {rowCells(row).map((val, j) => (
